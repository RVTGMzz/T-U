...
$integrator = Join-Path $root '_build_support\IntegrateAlpha631EquipmentRpgPolish.ps1'
$project = Join-Path $root 'src\TeamUp\TeamUp.csproj'
$manifest = Join-Path $root 'src\TeamUp\manifest.json'
$releaseDir = Join-Path $root 'release'
$stageRoot = Join-Path $root '_stage_alpha631'
$stageMod = Join-Path $stageRoot 'Team Up'
$log = Join-Path $root 'BUILD_LOG.txt'
$zip = Join-Path $releaseDir 'TeamUp_v0.2.0-alpha.6.3.1_EQUIPMENT_RPG_POLISH_TEST.zip'
$shaPath = Join-Path $releaseDir 'TeamUp_v0.2.0-alpha.6.3.1_EQUIPMENT_RPG_POLISH_TEST.sha256.txt'
...
try {
    if (-not (Test-Path $integrator)) { throw "Missing Alpha 6.3.1 integrator: $integrator" }
    ...
    Log 'Compiling Team Up Alpha 6.3.1...'
    ...
    if (-not (Test-Path $zip)) { throw 'Alpha 6.3.1 ZIP was not created.' }

    $hash = (Get-FileHash $zip -Algorithm SHA256).Hash.ToLowerInvariant()
    [System.IO.File]::WriteAllText($shaPath, "$hash  $(Split-Path $zip -Leaf)`r`n", $utf8NoBom)

    $smoke = Join-Path $root 'SMOKE_TEST_V0_2_ALPHA6_3_1_EQUIPMENT_RPG_VI.txt'
    ...
    Log ''
    Log '========================================================='
    Log 'BUILD SUCCESS - ALPHA 6.3.1'
    Log 'SMAPI MUST SHOW: Team Up DEBUG HARNESS READY ... 6.3.1'
    Log 'EQUIPMENT: RARITY + ROLE SCORE + COMBAT IMPACT + AUTO EQUIP'
    Log "ZIP: $zip"
    Log "SHA256: $hash"
    Log '========================================================='
}
