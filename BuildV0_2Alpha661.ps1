...
try {
    ...
    # -------------------- PartyManager: 6 people shared, 2 combat companions shared --------------------
...
'@
        $partyText = Replace-Required $partyText $old $new 'PartyManager.GetAnyOwner'
    }
...
'@
        $partyText = Replace-RegexRequired $partyText $pattern $replacement 'PartyManager shared-cap block'
    }
...
'@
    ...
    public CompanionUnitData? GetCompanionByUnitIdAnyOwner(string unitId)
        => _companionUnits.FirstOrDefault(unit =>
            string.Equals(unit.UnitId, unitId, StringComparison.OrdinalIgnoreCase));
'@
        $partyText = Replace-Required $partyText $old $new 'PartyManager global companion lookup'
    }
...
'@
        $partyText = Replace-Required $partyText '    public CompanionAddResult TryLinkCompanion(' ($insert + '    public CompanionAddResult TryLinkCompanion(') 'PartyManager player summon registration'
    }
...
'@
    $newGuard = @'
        if (state == CompanionDeploymentState.Active
            ...
            && !CanActivateAnotherCombatCompanion())
'@
    if ($partyText.Contains($oldGuard)) { $partyText = $partyText.Replace($oldGuard, $newGuard) }
...
'@
        $partyText = Replace-Required $partyText '    public bool SetState(string characterName, long recruiterId, PartyMemberState state)' ($insert + '    public bool SetState(string characterName, long recruiterId, PartyMemberState state)') 'PartyManager capacity enforcement'
    }
...
'@
        $partyText = Replace-Required $partyText $old $new 'PartyManager global companion cap'
    }
    [System.IO.File]::WriteAllText($party, $partyText, $utf8NoBom)

    # -------------------- FollowService: owner routing + PartyControlled handshake --------------------
...
'@
        $followText = Replace-Required $followText $old $new 'FollowService per-Farmer overload'
    }

    if (-not $followText.Contains('public void PrepareForParty(NPC npc, long? recruiterId = null)')) {
        $old = @'
...
'@
        $followText = Replace-Required $followText $old $new 'FollowService controlled marker'
    }

    if (-not $followText.Contains('npc.modData.Remove(PartyControlledModDataKey)')) {
        $followText = Replace-Required $followText "        RestoreFarmerPassThrough(npc);`n        npc.Halt();" "        RestoreFarmerPassThrough(npc);`n        npc.modData.Remove(PartyControlledModDataKey);`n        npc.modData.Remove(PartyControllerOwnerModDataKey);`n        npc.Halt();" 'FollowService release marker'
    }
...
'@
    ...
    private void FollowTarget
'@
        $followText = Replace-RegexRequired $followText $pattern $replacement 'FollowService owner companion route'
    }
...
'@
    if ($followText.Contains($oldFind)) { $followText = $followText.Replace($oldFind, $newFind) }
    [System.IO.File]::WriteAllText($follow, $followText, $utf8NoBom)

    # -------------------- CombatService: per-Farmer context --------------------
...
'@
        $combatText = Replace-Required $combatText "    private int _threatPulseTicks;`n    private int _lastFarmerHealth = -1;" $context.TrimEnd() 'CombatService Farmer context'
    }
    [System.IO.File]::WriteAllText($combat, $combatText, $utf8NoBom)

    # -------------------- ModEntry: host authority + client requests --------------------
    ...
    if (-not $modText.Contains('RegisterAlpha661MultiplayerEvents();')) {
        $modText = Replace-Required $modText '        helper.Events.Display.RenderedActiveMenu += OnRenderedActiveMenu;' "        helper.Events.Display.RenderedActiveMenu += OnRenderedActiveMenu;`n        RegisterAlpha661MultiplayerEvents();" 'ModEntry multiplayer event registration'
    }
...
'@
    ...
    if (-not $modText.Contains('UpdateOwnedPartyRuntime(e);')) {
        ...
        $modText = Replace-RegexRequired $modText $pattern '        UpdateOwnedPartyRuntime(e);' 'ModEntry owner runtime loop'
    }

    $oldButtonGuard = "        if (!Context.IsWorldReady || !Context.IsMainPlayer)`n            return;`n`n        if (Game1.activeClickableMenu is GameMenu gameMenu"
    $newButtonGuard = "        if (!Context.IsWorldReady)`n            return;`n`n        if (Game1.activeClickableMenu is GameMenu gameMenu"
    if ($modText.Contains($oldButtonGuard)) { $modText = $modText.Replace($oldButtonGuard, $newButtonGuard) }
...
'@
        $modText = Replace-RegexRequired $modText $pattern $replacement 'ModEntry recruit delegation'
    }
...
'@
        $modText = Replace-RegexRequired $modText $pattern $replacement 'ModEntry movement delegation'
    }
    ...
    $oldEquipment = '                case "Equipment": QueueUi(() => ShowEquipmentMenu(npc, member)); break;'
    $newEquipment = @'
                case "Equipment":
                    if (Context.IsMainPlayer)
                    ...
                    break;
'@
    if ($modText.Contains($oldEquipment)) { $modText = $modText.Replace($oldEquipment, $newEquipment.TrimEnd()) }
...
'@
    ...
    $strategyWithRemote = $strategyAnchor + "`n        ClearRemoteCombatServices();"
    if ($modText.Contains($strategyAnchor) -and -not $modText.Contains($strategyWithRemote)) {
        $modText = $modText.Replace($strategyAnchor, $strategyWithRemote)
    }

    $returnAnchor = "        Surge.Reset();`n        Party.Clear();"
    if ($modText.Contains($returnAnchor)) {
        $modText = $modText.Replace($returnAnchor, "        Surge.Reset();`n        ClearRemoteCombatServices();`n        Party.Clear();")
    }
    ...
    foreach ($token in @('PartyControlledModDataKey', 'PartyControllerOwnerModDataKey', 'Farmer owner')) {
        if (-not $followText.Contains($token)) { throw "Follow token missing: $token" }
    }
    foreach ($token in @('SetFarmerContext', 'FarmerContext.currentLocation', 'HardLeashTiles = 12f', 'TargetLockDurationTicks = 45', 'FacingHoldDurationTicks = 10')) {
        if (-not $combatText.Contains($token)) { throw "Combat token missing: $token" }
    }
    ...
    Log 'Recruit UX: NPC only / NPC + linked companion / cancel, with replacement flow at 2/2.'
    Log 'Multiplayer: host-authoritative requests and snapshots; per-Farmer Follow/Combat context.'
    Log 'Sudoku handshake: Ronvotri.TeamUp/PartyControlled=true while Team Up owns movement.'
