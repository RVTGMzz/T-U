...
$mod = Replace-RegexRequired $mod 'private void ShowMemberMenu\(NPC npc, PartyMemberData member\)\r?\n    \{\r?\n        RecruitHintNpcName = null;' "private void ShowMemberMenu(NPC npc, PartyMemberData member)`n    {`n        // Keep profile/leave shortcuts attached to the party-member root dialogue.`n        RecruitHintNpcName = npc.Name;" 'party member hints'
$mod = Replace-RegexRequired $mod 'private void ShowRoleMenu\(NPC npc, PartyMemberData member\)\r?\n    \{' "private void ShowRoleMenu(NPC npc, PartyMemberData member)`n    {`n        RecruitHintNpcName = null;" 'role submenu hint cleanup'
$mod = Replace-RegexRequired $mod 'private void ShowEngagementMenu\(NPC npc, PartyMemberData member\)\r?\n    \{' "private void ShowEngagementMenu(NPC npc, PartyMemberData member)`n    {`n        RecruitHintNpcName = null;" 'engagement submenu hint cleanup'
...
'@
$codex = Replace-RegexRequired $codex $updatePattern $updateReplacement 'native controller routing'
$codex = Replace-RegexRequired $codex 'public override void receiveGamePadButton\(Buttons b\)\r?\n    \{\r?\n        _showMouseCursor = false;' "public override void receiveGamePadButton(Buttons b)`n    {`n        _showMouseCursor = false;`n        _lastMousePosition = new Point(Game1.getMouseX(), Game1.getMouseY());" 'controller cursor hide'
$codex = Replace-RegexRequired $codex 'else if \(b == Buttons\.A\)\r?\n                return;' "else if (b == Buttons.A)`n            {`n                ApplyDropdownSelection();`n                return;`n            }" 'dropdown controller confirm'
$codex = Replace-RegexRequired $codex 'else if \(b == Buttons\.A\)\r?\n            return;' "else if (b == Buttons.A)`n        {`n            ActivateFocus();`n            return;`n        }" 'filter controller confirm'
