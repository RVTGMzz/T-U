...
$archive = Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.3.4_SMOKE_TEST.zip'
$log = Join-Path $root 'BUILD_LOG.txt'
$patchScript = Join-Path $root 'ApplyAlpha5_3_4Patches.ps1'

if (-not (Get-Command dotnet -ErrorAction SilentlyContinue)) { throw 'dotnet was not found. Install the .NET SDK first.' }
"Team Up alpha.5.3.4 build started: $(Get-Date -Format o)" | Set-Content $log
"dotnet: $(& dotnet --version)" | Add-Content $log

"Applying alpha.5.3.4 source hotfixes..." | Tee-Object -FilePath $log -Append
...
if (Test-Path $manifestBuilt) { Copy-Item $manifestBuilt $manifestDest }
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.1.0-alpha.5.3.4')
...
Copy-Item (Join-Path $root 'SMOKE_TEST_ALPHA5_3_4_VI.txt') (Join-Path $releaseDir 'SMOKE_TEST_ALPHA5_3_4_VI.txt')

if (Test-Path $archive) { Remove-Item $archive -Force }
Compress-Archive -Path $releaseDir -DestinationPath $archive -CompressionLevel Optimal
$hash = (Get-FileHash $archive -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.3.4_SMOKE_TEST.sha256.txt')
Write-Host ''
Write-Host '==============================================='
