...
$archive = Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.2_SMOKE_TEST.zip'
...
"Team Up alpha.5.2 build started: $(Get-Date -Format o)" | Set-Content $log
...
if (Test-Path $manifestBuilt) { Copy-Item $manifestBuilt $manifestDest }
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.1.0-alpha.5.2')
...
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.2_SMOKE_TEST.sha256.txt')
Write-Host ''
Write-Host '==============================================='
