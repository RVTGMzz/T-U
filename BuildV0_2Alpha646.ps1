...
$integrator = Join-Path $root '_build_support\IntegrateAlpha646ExpansionSignatureArtBalance.ps1'
$project = Join-Path $root 'src\TeamUp\TeamUp.csproj'
$manifest = Join-Path $root 'src\TeamUp\manifest.json'
$releaseDir = Join-Path $root 'release'
$stageRoot = Join-Path $root '_stage_alpha646'
$stageMod = Join-Path $stageRoot 'Team Up'
$log = Join-Path $root 'BUILD_LOG.txt'
$zip = Join-Path $releaseDir 'TeamUp_v0.2.0-alpha.6.4.6_EXPANSION_SIGNATURE_ART_BALANCE_TEST.zip'
$shaPath = Join-Path $releaseDir 'TeamUp_v0.2.0-alpha.6.4.6_EXPANSION_SIGNATURE_ART_BALANCE_TEST.sha256.txt'
...
try {
    if (-not (Test-Path $integrator)) { throw "Missing Alpha 6.4.6 integrator: $integrator" }
    if (-not (Test-Path $project)) { throw "Missing Team Up project: $project" }

    Log 'Integrating Alpha 6.4.6 Expansion Signature Art + Balance...'
    ...
    Log 'Compiling Team Up Alpha 6.4.6...'
    ...
    if (-not (Test-Path $zip)) { throw 'Alpha 6.4.6 ZIP was not created.' }

    $hash = (Get-FileHash $zip -Algorithm SHA256).Hash.ToLowerInvariant()
    [System.IO.File]::WriteAllText($shaPath, "$hash  $(Split-Path $zip -Leaf)`r`n", $utf8NoBom)

    $smoke = Join-Path $root 'SMOKE_TEST_V0_2_ALPHA6_4_6_EXPANSION_SIGNATURE_ART_BALANCE_VI.txt'
    ...
    Log ''
    Log '========================================================='
    Log 'BUILD SUCCESS - ALPHA 6.4.6'
    Log 'SMAPI MUST SHOW: Team Up DEBUG HARNESS READY ... 6.4.6'
    Log 'EXPANSION: ALL 51 FORMER PLACEHOLDERS KEEP REAL TIER 2/3 SIGNATURES'
    Log 'IDENTITY: ZERO-SUM POWER / REACH / UTILITY / TEMPO BUDGET PER NPC'
    Log 'BALANCE: EACH AXIS LIMITED TO -2..+2; POSITIVE STRENGTH REQUIRES WEAKNESS'
    Log 'ICONS: 51 SEMANTIC BESPOKE SIGNATURE MOTIFS; NO PROCEDURAL FALLBACK FOR COMPLETED ROSTER'
    Log 'REGRESSION: 6.4.5 EQUIPMENT / VAULT / AI / SOUND GUARDS RETAINED'
    Log "ZIP: $zip"
    Log "SHA256: $hash"
    Log '========================================================='
}
