...
$archive = Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.3_SMOKE_TEST.zip'
...
"Team Up alpha.3 build started: $(Get-Date -Format o)" | Set-Content $log
...
if (Test-Path $manifestBuilt) {
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.1.0-alpha.3')
...
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.3_SMOKE_TEST.sha256.txt')
