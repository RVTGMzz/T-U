...
///
/// Same-runtime-type creation remains the first choice in MonsterMutationMinionFactory. If a custom
/// runtime type cannot be safely reconstructed, the existing normal GreenSlime fallback is preserved.
/// </summary>
internal sealed class Alpha674416MutationLeaderMinionPolicyService
{
    ...
    public Alpha674416MutationLeaderMinionPolicyService(IMonitor monitor, string uniqueId)
    {
        ...
        _monitor.Log(
            "Team Up 6.7.44.16 Mutation leader/minion policy enabled: one Mutant leader + 2-4 ordinary hostile minions; only leader gets x3 loot.",
            LogLevel.Info);
    }
    ...
    public string Describe()
        => $"Mutation leader/minion policy: leader=Mutant | minions=normal-hostile | leaderLoot=x3 | minionLootBonus=none | "
            ...
            + $"sameType={_sameTypeSeen} | fallback={_fallbackSeen} | last={_last}";
    ...
    private void NormalizeMinion(Monster minion)
    {
        ...
        _last = $"minion type={minion.GetType().Name} ordinary=true hostileReady={ready} sameType={sameType} lootBonus=none";
    }
