...
STAGE = ROOT / "_stage_alpha6744_41"
MOD_STAGE = STAGE / "Team Up"
LOG = ROOT / "BUILD_LOG_ALPHA6744_41.txt"
VERSION = "0.2.0-alpha.6.7.44.41"
ZIP_NAME = "TeamUp_v0.2.0-alpha.6.7.44.41_CAPTURE_GUARD_SCOPE_FIX_TEST.zip"
ZIP_PATH = RELEASE / ZIP_NAME
SHA_PATH = RELEASE / "TeamUp_v0.2.0-alpha.6.7.44.41_CAPTURE_GUARD_SCOPE_FIX_TEST.sha256.txt"
lines: list[str] = []
...
try:
    ...
    log("VERSION IDENTITY 6.7.44.41: PASS")
    ...
    for rel in forbidden:
        req(not (SRC / rel).exists(), f"old crash-stack service leaked into 44.41: {rel}")
    ...
    for token in [
        ...
        "Alpha674440Version = \"0.2.0-alpha.6.7.44.41\"",
        "Alpha674440Branch = \"v0.2-alpha6-7-44-41-capture-guard-scope-fix\"",
    ]:
    ...
    req("string combined = typeKey + Normalize(method.Name);" not in capture,
        "capture guard still matches declaring type names")
    req("private static bool LooksLikeCaptureMethod(MethodInfo method)" in capture,
        "capture guard matcher is not method-name scoped")
    for blocked in ['"tostring"', '"printmembers"', '"gethashcode"', '"equals"', '"deconstruct"', '"dispose"', '"clone"']:
        req(blocked in capture, f"capture guard record/object exclusion missing: {blocked}")
    req("methodKey.Contains(\"capture\")" in capture
        and "methodKey.Contains(\"catch\")" in capture
        and "methodKey.Contains(\"pokeball\")" in capture,
        "capture guard capture/catch/pokeball method-name filter missing")
    log("CAPTURE GUARD METHOD-NAME SCOPE / RECORD EXCLUSION: PASS")
    ...
    log("BUILD SUCCESS - ALPHA 6.7.44.41 CAPTURE GUARD SCOPE FIX")
    log(f"ZIP: {ZIP_NAME}")
    log(f"SHA256: {digest}")
finally:
    LOG.write_text("\n".join(lines) + "\n", encoding="utf-8")
