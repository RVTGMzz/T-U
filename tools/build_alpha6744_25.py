...
STAGE = ROOT / "_stage_alpha6744_25"
MOD_STAGE = STAGE / "Team Up"
LOG = ROOT / "BUILD_LOG_ALPHA6744_25.txt"
VERSION = "0.2.0-alpha.6.7.44.25"
ZIP_NAME = "TeamUp_v0.2.0-alpha.6.7.44.25_LEADER_CONTINUOUS_CHASE_TEST.zip"
ZIP_PATH = RELEASE / ZIP_NAME
SHA_PATH = RELEASE / "TeamUp_v0.2.0-alpha.6.7.44.25_LEADER_CONTINUOUS_CHASE_TEST.sha256.txt"
lines: list[str] = []
...
try:
    ...
    log("PELIPPER FOLLOWER PACK + LEGACY LEADER STEERING CARRY-FORWARD: PASS")
    ...
    for token in [
        ...
        req(token in reach, f"6.7.44.24/25 reach-chain missing {token}")
    log("MUTANT LEADER 160PX REACH + 128PX HOLD CARRY-FORWARD: PASS")
    ...
    req("source.Halt()" not in continuous, "6.7.44.25 leader chase must not Halt() per movement step")
    req("source.Position =" not in continuous, "6.7.44.25 leader chase must not teleport source")
    req("controller = null" not in continuous and "temporaryController = null" not in continuous,
        "6.7.44.25 must not destroy provider controllers")
    ...
    req("MutationEliteFinalizationAlpha674424.Describe()" in mutation_cmd
        ...
        "elite telemetry missing from teamup_mutation status")
    log("MUTANT X2 DAMAGE PRESERVATION + LEADER CAPTURE GUARD: PASS")
    ...
    log("BUILD SUCCESS - ALPHA 6.7.44.25 LEADER CONTINUOUS CHASE")
    log(f"ZIP: {ZIP_NAME}")
    log(f"SHA256: {digest}")
finally:
    LOG.write_text("\n".join(lines) + "\n", encoding="utf-8")
