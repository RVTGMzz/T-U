...
# Register the read-only probe command.
...
entry = replace_once(
    ...
    "Alpha 6.7.22 registration",
)
entry_path.write_text(entry, encoding="utf-8", newline="\n")

probe = r'''using System.Collections;
...
probe_path = SRC / "Core" / "PelipperWildDensitySurfaceProbe.cs"
probe_path.write_text(probe, encoding="utf-8", newline="\n")

command = r'''using Ronvotri.TeamUp.Core;
...
command_path = SRC / "ModEntry.Alpha6722.cs"
command_path.write_text(command, encoding="utf-8", newline="\n")

print("Alpha 6.7.22 read-only Pelipper wild-density surface probe materialized.")
