...
if f"<Version>{VERSION}</Version>" not in project:
    if "<Version>0.2.0-alpha.6.7.8</Version>" not in project:
...
'''
write("Core/ChemistryVariantCatalog.cs", variant_catalog)

old_block = '''        PartyChemistryType chemistry = PartyChemistryCatalog.Resolve(first.Member.CharacterName, second.Member.CharacterName);
...
'''
new_block = '''        PartyChemistryType chemistry = PartyChemistryCatalog.Resolve(first.Member.CharacterName, second.Member.CharacterName);
        if (chemistry != PartyChemistryType.Neutral)
        {
...
replace_once(
    ...
    old_block,
    new_block,
    "ChemistryVariantCatalog.ResolveProfile(",
)
...
alpha679 = r'''using Ronvotri.TeamUp.Core;
...
'''
write("ModEntry.Alpha679.cs", alpha679)

print("Alpha 6.7.9 Chemistry Variants source materialized.")
