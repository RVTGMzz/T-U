...
STAGE = ROOT / "_stage_alpha6744_26"
MOD_STAGE = STAGE / "Team Up"
LOG = ROOT / "BUILD_LOG_ALPHA6744_26.txt"
VERSION = "0.2.0-alpha.6.7.44.26"
ZIP_NAME = "TeamUp_v0.2.0-alpha.6.7.44.26_PELIPPER_PHASE_LIFECYCLE_TEST.zip"
ZIP_PATH = RELEASE / ZIP_NAME
SHA_PATH = RELEASE / "TeamUp_v0.2.0-alpha.6.7.44.26_PELIPPER_PHASE_LIFECYCLE_TEST.sha256.txt"
lines: list[str] = []
...
try:
    ...
    log("ELITE REACH + CONTINUOUS CHASE + PHASE CHAIN: PASS")
    ...
    for token in [
        ...
        "PhaseTotalMarker",
        "PhaseCurrentMarker",
        "PhaseGuardTickMarker",
        "FinalLethalTickMarker",
        "AfterTryMutate",
        "BeforeSourceDamage",
        "AfterSourceDamage",
        "BeforeMonsterDrop",
        "current < total || guardedTransitionWindow",
        "prematureDropBlocks=",
        "finalDropPasses=",
        "restoreVerified=",
        "phase2=",
    ...
    req("MonsterMutationService.IsMutationMinion" not in phases,
        "phase lifecycle must not treat ordinary followers as Mutant leaders")
    req("new GreenSlime" not in phases, "phase lifecycle must not create fallback monsters")
    req("Position =" not in phases, "phase lifecycle must not move/teleport actors")
    ...
    log("BUILD SUCCESS - ALPHA 6.7.44.26 PELIPPER PHASE LIFECYCLE")
    log(f"ZIP: {ZIP_NAME}")
    log(f"SHA256: {digest}")
finally:
    LOG.write_text("\n".join(lines) + "\n", encoding="utf-8")
