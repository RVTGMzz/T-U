...
if f"<Version>{VERSION}</Version>" not in project:
    old = "<Version>0.2.0-alpha.6.7.4</Version>"
    if old not in project:
        raise RuntimeError("Unexpected TeamUp.csproj version for Alpha 6.7.5")
    write("TeamUp.csproj", project.replace(old, f"<Version>{VERSION}</Version>", 1))

catalog = r'''namespace Ronvotri.TeamUp.Core;
...
'''
write("Core/BanterContentCatalog.cs", catalog)

# Replace the old six-pair hardcoded method with a catalog-first lookup.
banter = read("Core/PartyBanterService.cs")
method_start = banter.index("    private Exchange BuildAmbientExchange(ActiveNpc first, ActiveNpc second, bool vi)\n")
method_end = banter.index("    private static Exchange OrderedPair", method_start)
new_method = r'''    private Exchange BuildAmbientExchange(ActiveNpc first, ActiveNpc second, bool vi)
    {
...
'''
banter = banter[:method_start] + new_method + banter[method_end:]

# Marlon is a positively known male special recruit; reflection still handles other custom males.
old_males = '        "Victor", "Lance", "Andy", "Martin", "Morris"\n'
new_males = '        "Victor", "Lance", "Andy", "Martin", "Morris", "Marlon"\n'
if new_males not in banter:
    if old_males not in banter:
        raise RuntimeError("KnownMaleNames anchor not found")
    banter = banter.replace(old_males, new_males, 1)

# Use authored MiMi pair openers/closers when the selected male pair has one.
shipping_start = banter.index("        bool vi = IsVietnamese();\n        string a = firstMale.Actor.displayName;", banter.index("private bool TryMimiShippingExchange"))
shipping_end = banter.index("    private static string BuildMimiShipResponse", shipping_start)
new_shipping = r'''        bool vi = IsVietnamese();
...
'''
banter = banter[:shipping_start] + new_shipping + banter[shipping_end:]
write("Core/PartyBanterService.cs", banter)

alpha675 = r'''using Ronvotri.TeamUp.Core;
...
'''
write("ModEntry.Alpha675.cs", alpha675)

alpha6625 = read("ModEntry.Alpha6625.cs")
if "EnsureAlpha675BanterCatalogRegistered();" not in alpha6625:
    anchor = "        // Alpha 6.7.4: static roster coverage/balance audit + one signature authority.\n        EnsureAlpha674RosterIntegrityRegistered();\n"
    ...
    if anchor not in alpha6625:
        raise RuntimeError("Alpha 6.7.4 registration anchor not found")
    write("ModEntry.Alpha6625.cs", alpha6625.replace(anchor, replacement, 1))

print("Alpha 6.7.5 banter depth source materialized.")
