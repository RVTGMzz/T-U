...
$archive = Join-Path $releaseRoot 'TeamUp_v0.2.0-alpha.1_COMBAT_TEST.zip'
...
$patchV02 = Join-Path $root 'ApplyV0_2Alpha1Patches.ps1'
...
"Team Up v0.2.0-alpha.1 build started: $(Get-Date -Format o)" | Set-Content $log
...
if ($modSource.Contains('v0.1.0-alpha.5.3.7 native vault + special lifecycle loaded.')) {
    ...
    & $patchV02 2>&1 | Tee-Object -FilePath $log -Append
    if ($LASTEXITCODE -ne 0) { throw 'v0.2-alpha.1 patch step failed.' }
}
elseif (-not $modSource.Contains('v0.2.0-alpha.1 full vanilla Codex + real NPC combat loaded.')) {
    throw 'Source is not at a recognized Team Up integration state.'
}
...
if (Test-Path $manifestBuilt) {
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.2.0-alpha.1')
...
$smoke = Join-Path $root 'SMOKE_TEST_V0_2_ALPHA1_VI.txt'
if (Test-Path $smoke) {
    Copy-Item $smoke (Join-Path $releaseDir 'SMOKE_TEST_V0_2_ALPHA1_VI.txt')
}
...
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.2.0-alpha.1_COMBAT_TEST.sha256.txt')

Write-Host ''
Write-Host '==============================================='
Write-Host 'BUILD SUCCESS - REAL NPC COMBAT CHECKPOINT'
