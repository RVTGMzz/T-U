...
Require ($native.Contains('HasVillagerLifecycle')) 'Native NPC availability boundary missing.'
Require ($native.Contains('HasPlayerLifecycle')) 'Native player availability boundary missing.'
...
Require ($a21.Contains('!routed && !nativeAvailable')) 'Bound native NPC lifecycle can still fall through to generic bridge.'
...
Require ($a13.Contains('if (nativeAvailable)')) 'Bound native player lifecycle can still fall through to actor fallback.'
Require ($a13.Contains('actor fallback suppressed')) 'Native player false-result guard missing.'
...
Log 'NATIVE NPC AUTHORITY: bound 1.1.9 failures never fall through to generic bridges.'
Log 'NATIVE PLAYER CALL/RETURN: ModEntry.DeployBesideOwner/RecallToBall wired.'
Log 'NATIVE PLAYER AUTHORITY: bound 1.1.9 failures never fall through to actor fields.'
...
Require ($dllStrings.Contains('HasVillagerLifecycle')) 'Native NPC availability boundary absent from DLL.'
Require ($dllStrings.Contains('HasPlayerLifecycle')) 'Native player availability boundary absent from DLL.'
