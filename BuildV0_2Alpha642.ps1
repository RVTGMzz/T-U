...
$integrator = Join-Path $root '_build_support\IntegrateAlpha642UiReadability.ps1'
$project = Join-Path $root 'src\TeamUp\TeamUp.csproj'
$manifest = Join-Path $root 'src\TeamUp\manifest.json'
$releaseDir = Join-Path $root 'release'
$stageRoot = Join-Path $root '_stage_alpha642'
$stageMod = Join-Path $stageRoot 'Team Up'
$log = Join-Path $root 'BUILD_LOG.txt'
$zip = Join-Path $releaseDir 'TeamUp_v0.2.0-alpha.6.4.2_UI_READABILITY_TEST.zip'
$shaPath = Join-Path $releaseDir 'TeamUp_v0.2.0-alpha.6.4.2_UI_READABILITY_TEST.sha256.txt'
...
try {
    if (-not (Test-Path $integrator)) { throw "Missing Alpha 6.4.2 integrator: $integrator" }
    if (-not (Test-Path $project)) { throw "Missing Team Up project: $project" }

    Log 'Integrating Alpha 6.4.2 UI Readability Pass...'
    ...
    Log 'Compiling Team Up Alpha 6.4.2...'
    ...
    if (-not (Test-Path $zip)) { throw 'Alpha 6.4.2 ZIP was not created.' }

    $hash = (Get-FileHash $zip -Algorithm SHA256).Hash.ToLowerInvariant()
    [System.IO.File]::WriteAllText($shaPath, "$hash  $(Split-Path $zip -Leaf)`r`n", $utf8NoBom)

    $smoke = Join-Path $root 'SMOKE_TEST_V0_2_ALPHA6_4_2_UI_READABILITY_VI.txt'
    ...
    Log ''
    Log '========================================================='
    Log 'BUILD SUCCESS - ALPHA 6.4.2'
    Log 'SMAPI MUST SHOW: Team Up DEBUG HARNESS READY ... 6.4.2'
    Log 'UI: PASSIVE + SIGNATURE DESCRIPTION TEXT = EXACTLY X2 BODY SCALE'
    Log 'UI: SHORT VIEWPORTS USE CLIPPED SCROLL REGION'
    Log 'INPUT: MOUSE WHEEL / UP-DOWN / GAMEPAD DPAD OR LEFT STICK'
    Log 'LOCKS: RELATIONSHIP + EQUIPMENT + SKILL IDENTITY + ONE SIGNATURE ICON RETAINED'
    Log "ZIP: $zip"
    Log "SHA256: $hash"
    Log '========================================================='
}
