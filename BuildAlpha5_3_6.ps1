...
$archive = Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.3.6_SMOKE_TEST.zip'
$log = Join-Path $root 'BUILD_LOG.txt'
$patchScript = Join-Path $root 'ApplyAlpha5_3_6Patches.ps1'

if (-not (Get-Command dotnet -ErrorAction SilentlyContinue)) {
    throw 'dotnet was not found. Install the .NET SDK first.'
}

"Team Up alpha.5.3.6 build started: $(Get-Date -Format o)" | Set-Content $log
"dotnet: $(& dotnet --version)" | Add-Content $log

"Applying alpha.5.3.6 integration patches..." | Tee-Object -FilePath $log -Append
& $patchScript 2>&1 | Tee-Object -FilePath $log -Append
...
if (Test-Path $manifestBuilt) {
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.1.0-alpha.5.3.6')
...
$smoke = Join-Path $root 'SMOKE_TEST_ALPHA5_3_6_VI.txt'
if (Test-Path $smoke) {
    Copy-Item $smoke (Join-Path $releaseDir 'SMOKE_TEST_ALPHA5_3_6_VI.txt')
}
...
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.3.6_SMOKE_TEST.sha256.txt')
