...
$archive = Join-Path $releaseRoot 'TeamUp_v0.2.0-alpha.2_COMBAT_VFX_TEST.zip'
...
"Team Up v0.2.0-alpha.2 build started: $(Get-Date -Format o)" | Set-Content $log
...
elseif (-not $modSource.Contains('v0.2.0-alpha.2 combat feedback + signature VFX loaded.')) {
...
if (Test-Path $manifestBuilt) {
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.2.0-alpha.2')
...
$smoke = Join-Path $root 'SMOKE_TEST_V0_2_ALPHA2_VI.txt'
if (Test-Path $smoke) {
    Copy-Item $smoke (Join-Path $releaseDir 'SMOKE_TEST_V0_2_ALPHA2_VI.txt')
}
...
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.2.0-alpha.2_COMBAT_VFX_TEST.sha256.txt')

Write-Host ''
Write-Host '==============================================='
Write-Host 'BUILD SUCCESS - COMBAT VFX CHECKPOINT'
