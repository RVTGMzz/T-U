...
if (-not $mod.Contains($newMarker)) {
    ...
    $mod = Replace-Required $mod `
        ...
        'initialize recruit progression'
...
'@
    $leaveGuardNew = @'
            ...
            bool removed = Party.Remove(npc.Name, recruiterId);
'@
    $mod = Replace-Required $mod $leaveGuardOld $leaveGuardNew 'safe equipment return before leave'
...
'@
    $roleNew = @'
            if (Party.SetRole(npc.Name, Game1.player.UniqueMultiplayerID, role))
            {
                ...
                SavePartyNow();
'@
    $mod = Replace-Required $mod $roleOld $roleNew 'role change progression normalize'
