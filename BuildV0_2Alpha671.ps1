...
$nativeChoiceOld = '        PelipperTownCompatibilityService.SetOwnerOptOut(owner, !includeCompanion);'
$nativeChoiceNew = @'
        PelipperTownCompatibilityService.SetOwnerOptOut(owner, !includeCompanion);

        // Alpha 6.7.1: the recruitment choice controls Pelipper at the source even when the
        // configured partner actor is asleep/dormant. NPC-only therefore cannot auto-spawn later.
...
$bridgeAnchor = '    public static bool TryIsVillagerCompanionConfiguredEnabled(string npcName, out bool enabled)'
$bridgeMethod = @'
    /// <summary>
...
if (-not $text.Contains('TryGetConfiguredVillagerCompanionDescriptor')) {
    ...
    $text = $text.Replace($bridgeAnchor, $bridgeMethod + $bridgeAnchor)
    WriteText $nativeBridge $text
}
...
$availableOld = '            int availablePelipperSlots = Math.Max(0, max - Math.Min(max, nonPelipper.Count));'
$availableNew = @'
...
$gusProfile = '["Gus"] = P("Gus", PartyRole.Support, PartyRole.Healer, EngagementStyle.Balanced, 3, 2, 5, 4, 2),'
$guntherProfile = $gusProfile + "`n            [\"Gunther\"] = P(\"Gunther\", PartyRole.Control, PartyRole.Support, EngagementStyle.Cautious, 2, 2, 4, 1, 5),"
...
$gusVisualAnchor = '        int healthBefore = target.Health;'
$gusVisual = @'
...
$defaultRecruitAnchor = '  "recruit.invite": "Recruit to Party",'
$defaultRecruitNew = @'
...
$viRecruitAnchor = '  "recruit.invite": "Thu nạp vào Party",'
$viRecruitNew = @'
...
$defaultGusAnchor = '  "codex.gus.ability": "Hot Plate: small recovery paired with a mid-range counterattack.",'
$defaultGunther = $defaultGusAnchor + "`n  \"codex.gunther.passive\": \"Curator's Eye: improves control value when enemies gather into a cluster.\",`n  \"codex.gunther.ability\": \"Relic Seal: seals a group of enemies with light damage and a reliable stun.\","
ReplaceOnce $defaultI18n $defaultGusAnchor $defaultGunther 'English Gunther profile text'

$viGusAnchor = '  "codex.gus.ability": "Hot Plate: hồi phục nhỏ kèm một đòn phản công tầm trung.",'
$viGunther = $viGusAnchor + "`n  \"codex.gunther.passive\": \"Curator's Eye: tăng hiệu quả Control khi kẻ địch tụ thành nhóm.\",`n  \"codex.gunther.ability\": \"Relic Seal: phong ấn một cụm quái, gây sát thương nhẹ và làm choáng ổn định.\","
