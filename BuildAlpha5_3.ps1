...
$archive = Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.3_SMOKE_TEST.zip'
...
"Team Up alpha.5.3 build started: $(Get-Date -Format o)" | Set-Content $log
...
if (Test-Path $manifestBuilt) { Copy-Item $manifestBuilt $manifestDest }
    ...
    $manifest = $manifest.Replace('%ProjectVersion%', '0.1.0-alpha.5.3')
...
Copy-Item (Join-Path $root 'SMOKE_TEST_ALPHA5_3_VI.txt') (Join-Path $releaseDir 'SMOKE_TEST_ALPHA5_3_VI.txt')

if (Test-Path $archive) { Remove-Item $archive -Force }
Compress-Archive -Path $releaseDir -DestinationPath $archive -CompressionLevel Optimal
$hash = (Get-FileHash $archive -Algorithm SHA256).Hash.ToLowerInvariant()
"$hash  $(Split-Path $archive -Leaf)" | Set-Content (Join-Path $releaseRoot 'TeamUp_v0.1.0-alpha.5.3_SMOKE_TEST.sha256.txt')
Write-Host ''
Write-Host '==============================================='
